...
void Matrix<T>::scatter(){
   ...
   if(mpiroot){		
      ...
      for(int i = 1; i < size; i++){
         for(long j = 0; j < dm; j++){
            MPI_Send(A[i*dm+j], n, mpitype, i, (i*dm+j), MPI_COMM_WORLD);
         }
      }	
      //---------------------------------------------------------------------------------------------------------------
   }else{
      ...
      for(long j = 0; j < dm; j++){
         MPI_Recv(loc.data[j], n, mpitype, 0, (rank*dm+j), MPI_COMM_WORLD, &status);
      }
   }
   //------------------------------------------------------------------------------------------------------------------
   MPI_Barrier(MPI_COMM_WORLD);
...
void Matrix<T>::gather(){
   ...
   if(mpiroot){
      ...
      for(int i = 1; i < size; i++){
         for(long j = 0; j < dm; j++){	
            MPI_Recv(A[i*dm+j], n, mpitype, i, (i*dm+j), MPI_COMM_WORLD, &status);
         }	
      }	
      //---------------------------------------------------------------------------------------------------------------
   }else{
      for(long j = 0; j < dm; j++){
         MPI_Send(loc.data[j], n, mpitype, 0, (rank*dm+j), MPI_COMM_WORLD);
      }
   }
   ...
   for(int i = 0; i < size-1; i++){	
      ...
      for(long j = 0; j < m2; j++){
         MPI_Sendrecv_replace(
            tmp.loc.data[j], n2, mpitype, 
            (rank) ? (rank-1) : (size-1), j,
            (rank == size-1) ? (0) : (rank+1), j, 
