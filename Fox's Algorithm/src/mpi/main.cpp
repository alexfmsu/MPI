...
void multiply(const char* file1, const char* file2, const char* file3){
   ...
   switch(type1){
      case 1:{
         ...
         int pMatrixAblock[A.get_block_size() * A.get_block_size()];
         A.scatter(grid_coords, pMatrixAblock, row_comm, col_comm);  
         //------------------------------------------------------------------------------------------------------------         
         if(mpiroot)cout << "Matrix B:" << endl << endl;
         Matrix<int> B;
         B.read_from_file(file2);
         if(mpiroot)cout << "-------------------------" << endl;
         B.scatter(grid_coords, B.A_block, row_comm, col_comm);         
         //------------------------------------------------------------------------------------------------------------
         if(mpiroot)cout << "Matrix C:" << endl << endl;
         Matrix<int> C(A.get_m(), B.get_m());
         
         mult(A, B, C, grid_coords, pMatrixAblock, row_comm, col_comm); 
      ...
      case 2:{
         ...
         A.read_from_file(file1);
         if(mpiroot)cout << "-------------------------" << endl;
         double pMatrixAblock[A.get_block_size() * A.get_block_size()];
         A.scatter(grid_coords, pMatrixAblock, row_comm, col_comm);  
         //------------------------------------------------------------------------------------------------------------         
         if(mpiroot)cout << "Matrix B:" << endl << endl;
         Matrix<double> B;
         ...
         //------------------------------------------------------------------------------------------------------------
         if(mpiroot)cout << "Matrix C:" << endl << endl;
         Matrix<double> C(A.get_m(), B.get_m());
      ...
      case 3:{
         ...
         A.read_from_file(file1);
         if(mpiroot)cout << "-------------------------" << endl;
         float pMatrixAblock[A.get_block_size() * A.get_block_size()];
         A.scatter(grid_coords, pMatrixAblock, row_comm, col_comm);  
         //------------------------------------------------------------------------------------------------------------         
         if(mpiroot)cout << "Matrix B:" << endl << endl;
         Matrix<float> B;
         ...
         //------------------------------------------------------------------------------------------------------------
         if(mpiroot)cout << "Matrix C:" << endl << endl;
         Matrix<float> C(A.get_m(), B.get_m());
